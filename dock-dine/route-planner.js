(() => {
  ...
  function easternDateString(ms = Date.now()) {
    const p = easternParts(ms); return `${p.year}-${p.month}-${p.day}`;
  }
  function easternMs(text) {
    ...
    for (let i=0; i<3; i++) {
      const p = easternParts(guess); const wall = Date.UTC(+p.year, +p.month-1, +p.day, +p.hour, +p.minute); guess += target - wall;
    }
    return guess;
  }
